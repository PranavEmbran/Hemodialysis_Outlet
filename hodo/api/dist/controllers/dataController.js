...
export const addVascularAccess = async (req, res) => {
    try {
        ...
        if (!access.VAL_Access_ID_PK)
            access.VAL_Access_ID_PK = Date.now();
...
export const updateVascularAccess = async (req, res) => {
    try {
        ...
        if (idx === -1)
            return res.status(404).json({ error: 'Vascular access type not found' });
...
export const addDialyzerType = async (req, res) => {
    try {
        ...
        if (!dialyzer.DTL_ID_PK)
            dialyzer.DTL_ID_PK = Date.now();
...
export const updateSchedulingLookup = async (req, res) => {
    try {
        ...
        if (idx === -1)
            return res.status(404).json({ error: 'Scheduling lookup entry not found' });
